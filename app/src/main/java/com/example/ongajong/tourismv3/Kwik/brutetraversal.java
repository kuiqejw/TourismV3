...
public class brutetraversal {
    // a public static class, that returns a linked list in the following form-> 0=LinkedList<com.example.ongajong.tourismv3.Kwik.node> 1=time taken 2=cost 3=Linkedlist, movetypes
    ...
    private node start; //saves the start com.example.ongajong.tourismv3.Kwik.node.
    ...
    public LinkedList<Object> findans(LinkedList<node> attractions, node start, double budget){
        //com.example.ongajong.tourismv3.Kwik.node start = start from where
        //com.example.ongajong.tourismv3.Kwik.node end = end where?
        //the purpose is to create a brute attack that looks for all possible permutations.
        this.master = attractions;
        this.budget=budget;
        //this.master.removeFirstOccurrence(start); //remove the start com.example.ongajong.tourismv3.Kwik.node from consideration
        node removal=null;
        ...
        //master does not contain start com.example.ongajong.tourismv3.Kwik.node.
        LinkedList<move> ans = new LinkedList<>();
        for (node moe: master){
            alllocations.put(moe.name,moe); //place each com.example.ongajong.tourismv3.Kwik.node into the master hashmap!
            //note that alllocations does not include the start com.example.ongajong.tourismv3.Kwik.node!
        }
        this.start =start;
    ...
    public LinkedList<move> recur(LinkedList<move> journey){
        ...
        if(journey.size()==0){//this is the home com.example.ongajong.tourismv3.Kwik.node, where the final answer is given.
            ...
            for(node a:this.master){
                ...
                com.example.ongajong.tourismv3.Kwik.debugprintjourney.print(hold);
                ...
                if(cost<=this.budget &&time<besttime){   //better in both ways..
                    ...
                    com.example.ongajong.tourismv3.Kwik.debugprintjourney.print(bestmoves);
            ...
            System.out.println("Branches: " + this.branches);*/
            return bestmoves;
        } //end of home com.example.ongajong.tourismv3.Kwik.node.
        ...
        //beginning of end com.example.ongajong.tourismv3.Kwik.node. i.e. we have visited all possible nodes!
        else if(journey.size()==this.master.size()){ //all possible nodes iterated!
            this.branches+=3;
            //System.out.println("Reached an end.");

            move secondlast = journey.getLast(); //get the last com.example.ongajong.tourismv3.Kwik.move that was made to determine the location!
            node lastvisited = alllocations.get(secondlast.towards); //obtain the location you were at.
            move bus =lastvisited.possible.get(start.name).get(movetype.BUSTR); //obtain the bustrain com.example.ongajong.tourismv3.Kwik.move to the start again
            move taxi =lastvisited.possible.get(start.name).get(movetype.TAXI); //obtain the taxi com.example.ongajong.tourismv3.Kwik.move to the start again
            move foot =lastvisited.possible.get(start.name).get(movetype.FOOT); //obtain the foot com.example.ongajong.tourismv3.Kwik.move to the start again
            ...
            /*System.out.println("Cost: " +bestcost);
            System.out.println("Time: " + besttime);
            com.example.ongajong.tourismv3.Kwik.debugprintjourney.print(journey);*/
            //com.example.ongajong.tourismv3.Kwik.debugprintjourney.print(journey);
            if(cost<this.budget && time<besttime){
            ...
            System.out.println("Cost: " +bestcost);
            System.out.println("Time: " + besttime);*/
            journey.removeLast(); //remove the last com.example.ongajong.tourismv3.Kwik.move.
            return bestmoves;
        ...
        //beginning of middling com.example.ongajong.tourismv3.Kwik.node.
            ...
            node lastplacenode = this.alllocations.get(lastplace); //get that com.example.ongajong.tourismv3.Kwik.node.
            /*for(com.example.ongajong.tourismv3.Kwik.node target: placestogo){
            ...
            for(node targets: placestogo){
                ...
                for(com.example.ongajong.tourismv3.Kwik.move asdd:journey){
               ...
               // com.example.ongajong.tourismv3.Kwik.debugprintjourney.print(journey);
                ...
                journey.add(taxi);  //com.example.ongajong.tourismv3.Kwik.move to next type.
                ...
                journey.add(foot); //com.example.ongajong.tourismv3.Kwik.move to next type.
                //System.out.println(journey.size() + " " + master.size());
